...
class DetailOrderMonth extends Component {
    ...
    getListOrderDetail = () => {
        ...
        axios.post(`${KONEKSI}/transaction/getlistordercustomerthismonth`,{
            month,year
        }).then((res) => {
    ...
    buttonBack = () => {
            return (<button className="btn btn-default orange float-left my-3" onClick={()=>{window.location=`history`}}>back</button>)
    }

    cookieCheck = () => {
        if(!cookies.get('myPengguna')){
            window.location='/'
        }
    }
